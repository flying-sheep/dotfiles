...
export def main [
  ...
  gh api --paginate -X GET notifications -q '[.[] | select(.subject.type == "PullRequest" and (.subject.title | test("pre-commit autoupdate$|bump\\b.*\\bgroup\\b"; "i")))]' | from json --objects | each { |threads|
